...
public class DetailActivity extends ActionBarActivity {
	...
	// "http://androideity.com/wp-content/uploads/2011/12/animacionframe.png";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// View tweetView =
		// getActivity().getLayoutInflater().inflate(R.layout.dialog_tweet,
		// null);
		...
		if (this != null) {
			if (Page_TimeLine.eventoActivo) {
				...
				SharedPreferences prefs = getSharedPreferences("latlong",
				...
				((TextView) findViewById(R.id.detallesTitulo)).setText(evento
						.getNombreEvento());
				imageloader.displayImage(evento.getUrlImagen(),
						(ImageView) findViewById(R.id.imagenHeader), options);
				((TextView) findViewById(R.id.detallesCategoria))
						.setText(evento.getCategoriaEvento());
				((TextView) findViewById(R.id.detallesFecha)).setText(evento
						.getFechaEvento());
				((TextView) findViewById(R.id.detallesLugar)).setText(evento
						.getLugarEvento());
				((TextView) findViewById(R.id.detallesDescripcion))
						.setText(evento.getDescripcion());
				((TextView) findViewById(R.id.detallesFuente)).setText(evento
						.getFuente());
				((TextView) findViewById(R.id.detallesDireccion))
						.setText(evento.getDireccion());
				((TextView) findViewById(R.id.detallesTelefono)).setText(evento
						.getTelefono());
				((TextView) findViewById(R.id.detallesPrecio)).setText(evento
						.getBoleto());
				((TextView) findViewById(R.id.detallesDistancia)).setText("A "
						+ String.valueOf(evento.getDistancia()));
				tel = (TextView) findViewById(R.id.detallesTelefono);
				tel.setText(evento.getTelefono());

				String inicioTweet = getResources().getString(
						R.string.cadenaTweet_inicio);
				String enTweet = getResources().getString(
						R.string.cadenaTweet_en);
				final String tweetString = inicioTweet + " \""
						+ evento.getNombreEvento() + "\" " + enTweet + " "
						+ evento.getLugarEvento() + " Vía yiepa!";
				...
				if (Page_TimeLine.prendeEstrellaTime_Line[favoritosObjeto
						.getPosicion()]) {
					btnF.setImageResource(R.drawable.ic_action_important_active);
				} else {
					BitmapFactory.Options options = new BitmapFactory.Options();
				...
				((TextView) findViewById(R.id.detallesTitulo))
						.setText(favoritosObjeto.getNombreEvento());
				imageloader.displayImage(favoritosObjeto.getUrlImagen(),
						(ImageView) findViewById(R.id.imagenHeader), options);
				((ImageView) findViewById(R.id.iconCategoria))
						.setImageResource(favoritosObjeto.getImagenCategoria());
				((TextView) findViewById(R.id.detallesCategoria))
						.setText(favoritosObjeto.getCategoriaEvento());
				((TextView) findViewById(R.id.detallesFecha))
						.setText(favoritosObjeto.getFechaEvento());
				((TextView) findViewById(R.id.detallesLugar))
						.setText(favoritosObjeto.getLugarEvento());
				((TextView) findViewById(R.id.detallesDescripcion))
						.setText(favoritosObjeto.getDescripcion());
				((TextView) findViewById(R.id.detallesFuente))
						.setText(favoritosObjeto.getFuente());
				((TextView) findViewById(R.id.detallesDireccion))
						.setText(favoritosObjeto.getDireccion());
				((TextView) findViewById(R.id.detallesTelefono))
						.setText(favoritosObjeto.getTelefono());
				((TextView) findViewById(R.id.detallesPrecio))
						.setText(favoritosObjeto.getBoleto());
				((TextView) findViewById(R.id.detallesDistancia)).setText("A "
				...
				String enTweet = getResources().getString(
						R.string.cadenaTweet_en);
				final String tweetString = inicioTweet + " \""
						+ favoritosObjeto.getNombreEvento() + "\" " + enTweet
						+ " " + favoritosObjeto.getLugarEvento()
						+ " Vía yiepa!";

				String imageHttpAddress = "http://maps.googleapis.com/maps/api/staticmap?"
						+ "center="
						+ favoritosObjeto.getLatEvento()
						+ ","
						+ favoritosObjeto.getLonEvento()
						+ ""
						+ "&zoom=15"
						+ "&size=600x300"
						+ "&scale=2"
			...
			((RelativeLayout) findViewById(R.id.botonFavEvent))
			...
			((RelativeLayout) findViewById(R.id.capaMapa))
					.setOnClickListener(new OnClickListener() {
						@Override
						public void onClick(View v) {
							...
							if (Page_TimeLine.eventoActivo) {
								bundle.putDoubleArray("LatLon",
										new double[] { evento.getLonEvento(),
												evento.getLatEvento() });
							} else if (Page_TimeLine.favoritoActivo) {
								bundle.putDoubleArray("LatLon", new double[] {
										favoritosObjeto.getLonEvento(),
										favoritosObjeto.getLatEvento() });
							}
	...
	private void lanzarAlertDialogTweet(final String tweetString,
			final ImageView btnRet) {
		// Se coloca el OnClickListener al boton de retweet el cual crea un
		// dialogo
		// ((RelativeLayout)eventView.findViewById(R.id.botonRetweetEvent)).setOnClickListener(new
		// OnClickListener() {
	...
	private void agregarFavoritos() {
		...
		if (Page_TimeLine.eventoActivo) {
			...
			if (Page_TimeLine.prendeEstrellaTime_Line[evento.getPosicion()]) {
				...
				Log.d(null,
						"Se elimino Registro en DB " + evento.getNombreEvento());
				...
				manager.insertar(evento.getNombreEvento(),
						evento.getCategoriaEvento(), evento.getFechaEvento(),
						evento.getDescripcion(), evento.getFuente(),
						evento.getLugarEvento(), evento.getDireccion(),
						evento.getTelefono(), evento.getBoleto(),
						String.valueOf(evento.getDistancia()),
						String.valueOf(evento.getLatEvento()),
						String.valueOf(evento.getLonEvento()),
						evento.getUrlImagen(),
			...
			if (Page_TimeLine.prendeEstrellaTime_Line[favoritosObjeto
					.getPosicion()]) {
				// Si esta prendida solo apaga y elimina el registro
				manager.eliminar(String.valueOf(favoritosObjeto
						.getIndexOfEvent()));

				Log.d(null,
						"Se elimino Registro en DB "
								+ favoritosObjeto.getNombreEvento());
				// Apagamos estrella en timeline
				Page_TimeLine.prendeEstrellaTime_Line[favoritosObjeto
						.getPosicion()] = false;
	...
	private void refreshFavoritesFragment() {
		...
		if (fragment != null) {
			...
			fragmentManager.beginTransaction()
					.replace(R.id.content_Favorites, fragment).commit();
		}
	}
